...
export class RegisterUserComponent implements OnInit{
    ...
    if (this.registerForm.valid) {
      this.authService.register(this.registerForm.value).subscribe(
        (response: any) => {
          ...
          this.router.navigate(['/loginuser']); // Redirigir al login después de registro exitoso
        },
        (error: any) => {
  ...
  logOut() {
    this.router.navigate(['/loginuser']); // Redirigir al login cuando se hace clic en el icono de cerrar sesión
  }
}
